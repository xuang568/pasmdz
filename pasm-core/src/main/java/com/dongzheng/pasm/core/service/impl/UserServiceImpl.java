...
import com.dongzheng.pasm.core.dao.IUserDao;
...
public class UserServiceImpl extends BaseServiceImpl<User, Integer> implements IUserService {
    ...
    public void saveOrUpdate(User user) {
        if (user.getId() != null) {
            ...
            user.setPassword(MD5Utils.enCode("000000"));
            save(user);
        }
    }
    ...
    public void updatePwd(User user, String oldPassword, String password1, String password2) {
        ...
        Assert.isTrue(user.getPassword().equals(MD5Utils.enCode(oldPassword)), "原始密码不正确");
        ;
        Assert.isTrue(password1.equals(password2), "两次密码不一致");
        dbUser.setPassword(MD5Utils.enCode(password1));
